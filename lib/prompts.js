...
/** The Template system instruction. `strict` is used for the one retry after a response that failed to parse. */
function buildTemplateSystemInstruction(strict) {
  const reminder = strict
    ? 'Your previous reply could not be read as valid JSON in the required shape. This time, output ONLY the JSON object below — nothing else, no markdown fences, no commentary.\n\n'
    : '';

  return [
    reminder +
      'You are a template-field detector embedded in a browser extension. You look at a snippet of text and point out which parts are likely to change the next time this exact text gets reused — a name, a date, an order number, an amount, a place — without rewriting anything.',
    '',
    'Respond with exactly one JSON object, and nothing else, in this exact shape:',
    TEMPLATE_JSON_SHAPE,
    '',
    'Rules:',
    block([
      '"start" and "end" are 0-based character offsets into the text between the markers below (end exclusive), pointing at the exact span to mark.',
      '"label" is a short, human-readable name for the field (e.g. "Name", "Date", "Order number").',
      'Only mark spans that would genuinely differ the next time this text is reused. Never mark words that are part of the fixed wording.',
      'Spans must not overlap.',
      'If nothing in the text looks reusable-but-variable, return an empty "fields" array.',
      'Never change, reorder or add to the text itself — you are only pointing at spans within it.',
      'Do not wrap the output in quotation marks, backticks or markdown code fences.',
      'Do not explain anything before or after the JSON.',
      'Output the JSON object and nothing else.'
    ])
 ...
 * @param {{ strict?: boolean }} [opts]  strict: Coach/Template's one stricter retry after a malformed reply
...
export function buildSystemInstruction(action, settings, opts = {}) {
  ...
  if (action === 'template') {
    return buildTemplateSystemInstruction(Boolean(opts.strict));
  }
 ...
 *
 * @param {'grammar'|'translate'|'coach'|'template'} [action]  coach and template are annotated, not
 *   rewritten; everything else is a rewrite
 */
export function buildUserPrompt(text, action) {
  const instruction =
    action === 'coach'
      ...
      : action === 'template'
        ? 'Here is the selected text, between the markers. Find the fields in it according to your instructions and output only the JSON, without the markers.'
