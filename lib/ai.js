 ...
 * @returns {Promise<{ text: string, via: 'gemini' }>} the replacement and
 *   which provider produced it. Only Gemini today; the shape is what a second
 *   provider (on-device translate) slots into.
 */
export async function runAction(action, text, settings) {
  if (!settings.apiKey) {
    throw new AiError('NO_API_KEY', 'No Gemini API key set. Open Kalam settings to add one.');
  }
  ...
  // One deadline for the whole action, however many variants get tried and
  // whether or not the model has to be healed on the way.
  const deadline = new AbortController();
  const timer = setTimeout(() => deadline.abort(), TIMEOUT_MS);
  let raw;
  try {
    raw = await callWithLeastThinking(settings, body, deadline.signal);
  } catch (err) {
    if (!(err instanceof AiError && err.code === 'BAD_MODEL')) throw err;
    const healed = await healModel(settings, deadline.signal);
    if (!healed) throw err;
    raw = await callWithLeastThinking(healed, body, deadline.signal);
  } finally {
    clearTimeout(timer);
  }
...
function timeoutError() {
  return new AiError('TIMEOUT', 'Kalam took too long to respond. Try again.');
}
 ...
 */
export async function listModels(settings, { force = false } = {}) {
  if (!settings.apiKey) return FALLBACK_LIST;

  if (!force) {
    const cached = await readModelCache(settings.apiKey);
  ...
  const timer = setTimeout(() => deadline.abort(), TIMEOUT_MS);
  try {
    return await refreshModelList(settings.apiKey, deadline.signal);
  } catch {
...
/**
 * Filter to models that can generate text, order Flash first (newest generation
 * floating up within each group) and mark the recommended one: the hardcoded
 * default when the key still offers it, otherwise the first Flash model — or,
 * with no Flash at all, the first model, since anything beats a retired id.
 */
function shapeModels(raw) {
  ...
  const flash = usable.filter((m) => isFlash(m.id)).sort(newestFirst);
  const rest = usable.filter((m) => !isFlash(m.id)).sort(newestFirst);
  const models = [...flash, ...rest];

  const recommended = models.find((m) => m.id === DEFAULTS.model) || flash[0] || models[0];
...
function mapHttpError(status, payload) {
  ...
  if (status === 400 && /API[_ ]key/i.test(detail)) {
    return new AiError('BAD_KEY', 'That Gemini API key was rejected. Check it in Kalam settings.');
  }
  if (status === 400) {
