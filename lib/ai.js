 ...
 * on-device result.
 *
 * @param {'grammar'|'translate'|'coach'|'template'} action
 * @param {string} text  the exact text the user selected
 * @param {object} settings
 * @returns {Promise<{ text: string, via: 'gemini' | 'on-device', fields?: Array<{start:number,end:number,label:string}> }>}
 *   the replacement (unchanged input, for template) and which route produced it; `fields` is only
 *   present for template
 */
...
async function runGemini(action, text, settings, signal) {
  ...
  if (action === 'coach') return runCoach(text, settings, signal);
  if (action === 'template') return runTemplate(text, settings, signal);

  const body = rewriteBody(action, text, settings);
  const raw = await callHealing(settings, body, signal);
...
function rewriteBody(action, text, settings) {
  return {
    systemInstruction: {
    ...
    contents: [
      {
        role: 'user',
        parts: [{ text: buildUserPrompt(text, action) }]
      }
    ],
    generationConfig: {
...
/** One call to Gemini, healing a BAD_MODEL once (ADR 0004) by refreshing the list, saving the recommended model and retrying with it. */
async function callHealing(settings, body, signal) {
  try {
    return await callWithLeastThinking(settings, body, signal);
  } catch (err) {
    if (!(err instanceof AiError && err.code === 'BAD_MODEL')) throw err;
    const healed = await healModel(settings, signal);
    if (!healed) throw err;
    return callWithLeastThinking(healed, body, signal);
  }
}

/* --------------------------------------------------------------- coach */

const COACH_LEVELS = new Set(['weak', 'fair', 'good', 'strong']);

/**
 * When neither the first attempt nor the stricter retry parses as the
 * required shape (ADR 0007): a content-shape failure, never surfaced as
 * TIMEOUT or SERVER. Kept generic on purpose — no per-category guesswork
 * from unstructured text.
 */
const DEGRADED_COACH_RESULT = Object.freeze({
  overall: {
    level: 'fair',
    note: "Kalima couldn't produce a full breakdown this time. Try again, or use Fix to correct grammar directly."
  },
  categories: []
});

function isCoachCategory(c) {
  return (
    c &&
    typeof c.name === 'string' &&
    c.name &&
    COACH_LEVELS.has(c.level) &&
    typeof c.note === 'string' &&
    (c.spans === undefined ||
      (Array.isArray(c.spans) && c.spans.every((s) => Number.isInteger(s?.start) && Number.isInteger(s?.end))))
  );
}

/** True for a well-formed Coach payload: exactly the four categories, each shaped right. */
function isCoachPayload(obj) {
  return Boolean(
    obj &&
      obj.overall &&
      COACH_LEVELS.has(obj.overall.level) &&
      typeof obj.overall.note === 'string' &&
      Array.isArray(obj.categories) &&
      obj.categories.length === 4 &&
      obj.categories.every(isCoachCategory)
  );
}

/**
 * Parse one Coach reply. Returns `{ nonEnglish: true }`, a valid payload, or
 * null for anything that fails to parse or match the shape — the caller
 * decides whether that means "retry" or "degrade".
 */
function parseCoachReply(raw, text) {
  let obj;
  try {
    obj = JSON.parse(cleanOutput(raw, text));
  } catch {
    return null;
  }
  if (obj && obj.nonEnglish === true) return { nonEnglish: true };
  return isCoachPayload(obj) ? obj : null;
}

/**
 * Coach: one JSON-only request for a four-category writing breakdown (ADR
 * 0007), never a rewrite, so it never touches cleanOutput's rewrite-shaped
 * output the way grammar/translate's result does — only to strip stray
 * fences before JSON.parse. A reply that fails to parse as the required
 * shape gets one retry with a stricter reminder; a second failure degrades
 * to an overall-only result rather than surfacing as an error, because a
 * content-shape failure is not a network one.
 */
async function runCoach(text, settings, signal) {
  const attempt1 = await callHealing(settings, coachBody(text, settings, false), signal);
  let result = parseCoachReply(attempt1, text);
  if (!result) {
    const attempt2 = await callHealing(settings, coachBody(text, settings, true), signal);
    result = parseCoachReply(attempt2, text);
  }
  if (result?.nonEnglish) {
    throw new AiError('NON_ENGLISH', 'Coach works in English only for now.');
  }
  return { text: JSON.stringify(result || DEGRADED_COACH_RESULT), via: 'gemini' };
}

function coachBody(text, settings, strict) {
  return {
    systemInstruction: {
      parts: [{ text: buildSystemInstruction('coach', settings, { strict }) }]
    },
    contents: [
      {
        role: 'user',
        parts: [{ text: buildUserPrompt(text, 'coach') }]
      }
    ],
    generationConfig: {
      temperature: 0.3,
...
// Stricter than Coach's span check (isCoachCategory): a Coach span is only
// ever a highlight hint the panel clamps and forgives, but a Template field
// gets persisted and later drives fillTemplate's substitution, so a bad
// offset here would corrupt a saved template rather than just mis-render
// one tooltip — worth bounding and rejecting overlaps rather than trusting.
function isTemplateField(f, maxLen) {
  return Boolean(
    f &&
...
/** True if any two fields (sorted by start) overlap — the prompt's own "spans must not overlap" rule, enforced rather than trusted. */
function fieldsOverlap(fields) {
  const sorted = [...fields].sort((a, b) => a.start - b.start);
  return sorted.some((f, i) => i > 0 && f.start < sorted[i - 1].end);
}

function isTemplatePayload(obj, maxLen) {
  return Boolean(
    obj && Array.isArray(obj.fields) && obj.fields.every((f) => isTemplateField(f, maxLen)) && !fieldsOverlap(obj.fields)
  );
}

/** Parse one Template reply into a fields array, or null for anything that fails to parse or match the shape. */
function parseTemplateReply(raw, text) {
  let obj;
  try {
    obj = JSON.parse(cleanOutput(raw, text));
  } catch {
  ...
  return isTemplatePayload(obj, text.length) ? obj.fields : null;
}

/**
 * Template: one JSON-only request for the spans likely to vary next time
 * this text is reused, never a rewrite: `text` in the result is always the
 * unmodified input, so the caller can trust it byte-for-byte regardless of
 * what the model returns. A reply that fails to parse as the required
 * shape gets one retry with a stricter reminder; a second failure degrades
 * to an empty field list rather than an error — the action still succeeds,
 * the user just marks fields by hand instead of correcting suggestions.
 */
async function runTemplate(text, settings, signal) {
  const attempt1 = await callHealing(settings, templateBody(text, settings, false), signal);
  let fields = parseTemplateReply(attempt1, text);
  if (fields === null) {
    const attempt2 = await callHealing(settings, templateBody(text, settings, true), signal);
    fields = parseTemplateReply(attempt2, text);
  }
  return { text, via: 'gemini', fields: fields || [] };
}

function templateBody(text, settings, strict) {
  return {
    systemInstruction: {
      parts: [{ text: buildSystemInstruction('template', settings, { strict }) }]
    },
    contents: [
      {
        role: 'user',
        parts: [{ text: buildUserPrompt(text, 'template') }]
      }
    ],
    generationConfig: {
      temperature: 0.2,
