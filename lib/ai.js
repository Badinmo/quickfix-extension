...
export async function runAction(action, text, settings) {
  ...
  // One deadline for the whole action, however many variants get tried.
...
/** @param {AbortSignal} signal  the action's deadline; aborting it ends the request as TIMEOUT */
async function callGemini(model, apiKey, body, signal) {
  ...
  // timeout too, not an empty response. Anything else unparseable is mapped below.
  let payload = null;
  try {
