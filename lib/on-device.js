...
export async function translateOnDevice(text, settings, signal) {
  ...
  try {
    ...
    // action past the deadline: the race ends the promise Kalima is waiting on
...
export async function onDeviceStatus({ sourceLanguage, targetLanguage }) {
  ...
  if (unknown) {
    return { supported: false, availability: 'unavailable', reason: `Kalima has no language code for ${unknown}, so the browser cannot be asked about ${pairName}.` };
  }
  ...
  switch (availability) {
    ...
    default:
      return { supported: false, availability, reason: `The browser answered "${availability}" for ${pairName}, which Kalima does not know how to use.` };
  }
}
...
export async function downloadOnDeviceLanguage({ sourceLanguage, targetLanguage }, { onProgress, signal } = {}) {
  ...
  if (!source || !target) {
    throw new Error(`Kalima has no language code for ${!source ? sourceLanguage : targetLanguage}.`);
  }
