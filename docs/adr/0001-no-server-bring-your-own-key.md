...
Kalima runs no backend. Each user supplies their own provider key (one per person), stored in
...
- **Hosted key + subscription** (the consumer norm): needs a server, accounts, billing, abuse
  prevention, and makes Kalima a data processor. Rejected for now: don't build a business before
  there are users. Kept open as a future provider ("Kalima Cloud"), not as the default.
